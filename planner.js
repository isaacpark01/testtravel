/* ============================================================
   ...
   Storage: localStorage key "roamly_v2"
...
function shareTripPlan() {
  ...
  lines.push('✨ Planned with Roamly');
...
let activeLang = localStorage.getItem('roamly_lang') || 'en';
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (new URLSearchParams(location.search).get('reset') !== null) {
    localStorage.removeItem('roamly_v2');
...
function openAIChat() {
  ...
  if (!msgs.children.length) _addAIMsg('assistant', _aiWelcome());
  AI.detect().then(p => _setAIBadge(p));
  setTimeout(() => document.getElementById('ai-input').focus(), 80);
}

function closeAIChat() {
  document.getElementById('ai-panel').classList.add('hidden');
  document.getElementById('ai-fab').style.display = '';
}

function _aiWelcome() {
  if (!currentTrip) return 'Hi! I\'m your AI travel assistant. Create a trip to get personalised advice, or ask me anything about travel!';
  const city = CITIES.find(c => c.id === currentTrip.cityId);
  const cn = city ? city.name : currentTrip.cityId;
  return `Hi! Ready to help plan your trip to **${cn}**. Use the quick buttons or ask me anything — day plans, food picks, hidden gems, budget tips, or how to get around!`;
}

function _buildSystemPrompt() {
  let sys = `You are a concise, knowledgeable travel assistant for Roamly. Help the user plan their trip.

When you suggest a specific place, restaurant, or attraction, format it exactly like this on its own line:
[SUGGEST: Place Name | $price | type]
where type is one of: activity, food, transport
Example: [SUGGEST: Senso-ji Temple | $0 | activity]
Example: [SUGGEST: Ichiran Ramen | $15 | food]

Keep responses to 3-6 sentences unless the user asks for a detailed list. Be practical and specific.`;

  if (!currentTrip) return sys;
  const city = CITIES.find(c => c.id === currentTrip.cityId);
  const days  = (currentTrip.days || []).length;
  const perDay = currentTrip.budget ? Math.round(currentTrip.budget / Math.max(days, 1)) : null;
  const planned = (currentTrip.days || []).flatMap(d => (d.cards || []).map(c => c.name)).slice(0, 12);
  sys += `\n\nCurrent trip:\n- Destination: ${city ? city.name : currentTrip.cityId}\n- Days planned: ${days}\n- Budget: ${perDay ? `$${currentTrip.budget} total ($${perDay}/day)` : 'not set'}\n- Already planned: ${planned.length ? planned.join(', ') : 'nothing yet'}`;
  return sys;
}
...
async function sendAIMessage(text) {
  ...
  _addAIMsg('user', msg);
  const asst = _addAIMsg('assistant', '');
  const body = asst.querySelector('.ai-msg-content');
  ...
  await AI.stream(
    ...
    (chunk) => {
      full += chunk;
      body.innerHTML = _renderAIMd(full) + '<span class="ai-cursor">&#9607;</span>';
      _aiScrollBottom();
    },
    () => {
      body.innerHTML = _renderAIMd(full);
      _renderAISuggestions(body, full);
    ...
    (err) => {
      body.innerHTML = `<span style="color:#f87171">&#9888; ${esc(err)}</span><br><small style="color:#475569">Open &#9881; Settings to configure an AI provider.</small>`;
      _aiStreaming = false;
      document.getElementById('ai-send-btn').disabled = false;
    }
  );
...
function _addAIMsg(role, text) {
  ...
  div.innerHTML = `<div class="ai-msg-content">${role === 'user' ? esc(text) : _renderAIMd(text)}</div>`;
  msgs.appendChild(div);
  _aiScrollBottom();
  return div;
}

function _renderAIMd(text) {
  return text
    .replace(/\[SUGGEST:[^\]]*\]/g, '')
    .replace(/\*\*([^*\n]+)\*\*/g, '<strong>$1</strong>')
    .replace(/\*([^*\n]+)\*/g, '<em>$1</em>')
...
function _renderAISuggestions(container, text) {
  const rx = /\[SUGGEST:\s*([^|]+)\|\s*\$?(\d*)\s*\|\s*(\w+)\s*\]/g;
  ...
  matches.forEach(m => {
    ...
    const emoji = type === 'food' ? '\uD83C\uDF7D' : type === 'transport' ? '\uD83D\uDE87' : '\uD83C\uDFAF';
    const card  = document.createElement('div');
    card.className = 'ai-suggestion-card';
    card.innerHTML = `<div class="ai-sug-name">${esc(name)}</div><div class="ai-sug-meta">${emoji} ${esc(type)} &middot; ${price === 0 ? 'Free' : '$' + price}</div><button class="ai-sug-add" onclick="aiAddSuggestion('${jsq(name)}','${jsq(type)}',${price})">+ Add to Saves</button>`;
    wrap.appendChild(card);
  });
  container.after(wrap);
}

function aiAddSuggestion(name, type, price) {
  if (!currentTrip) { showToast('Create a trip first!'); return; }
  const city = CITIES.find(c => c.id === currentTrip.cityId);
  savePlace({
    id: crypto.randomUUID(), name, type,
    category: type === 'food' ? 'food' : 'activity',
    price: parseInt(price) || 0,
    cityId:   currentTrip.cityId,
    cityName: city ? city.name : currentTrip.cityId,
    socialSource: 'ai',
  });
  showToast(esc(name) + ' added to saves!');
}
...
function openAISettings() {
  const cfg = AI.cfg;
  document.getElementById('ai-set-provider').value   = cfg.provider;
  document.getElementById('ai-set-groq-key').value   = cfg.groqKey;
  document.getElementById('ai-set-gemini-key').value = cfg.geminiKey;
  document.getElementById('ai-set-groq-model').value = cfg.groqModel;
  const gs = document.getElementById('ai-set-gemini-model');
  if (gs) gs.value = cfg.geminiModel;
  toggleAISettingsFields(cfg.provider);
...
function toggleAISettingsFields(provider) {
  document.getElementById('ai-groq-fields').style.display   = (provider === 'groq'   || provider === 'auto') ? '' : 'none';
  document.getElementById('ai-gemini-fields').style.display = (provider === 'gemini' || provider === 'auto') ? '' : 'none';
}

function saveAISettings() {
  const cfg = AI.cfg;
  cfg.provider  = document.getElementById('ai-set-provider').value;
  cfg.groqKey   = document.getElementById('ai-set-groq-key').value.trim();
  cfg.geminiKey = document.getElementById('ai-set-gemini-key').value.trim();
  cfg.groqModel = document.getElementById('ai-set-groq-model').value;
  const gs = document.getElementById('ai-set-gemini-model');
  if (gs) cfg.geminiModel = gs.value;
  AI.saveCfg();
  AI.resetDetected();
  closeAISettings();
  AI.detect().then(p => _setAIBadge(p));
...
function _setAIBadge(p) {
  const b = document.getElementById('ai-provider-badge');
  if (!b) return;
  const labels = { ollama: '\uD83D\uDFE2 Ollama', groq: '\u26A1 Groq', gemini: '\u2726 Gemini', none: '\u26A0 No provider' };
