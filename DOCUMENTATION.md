...
- **Storage:** `localStorage` key `roamly_v2` (planner state persists across sessions)
...
| Function | Purpose |
|---|---|
| `getPlans()` | Reads `roamly_v2` from localStorage (returns `{trips:[]}` default) |
| `savePlans(data)` | Writes `roamly_v2` to localStorage |
...
| Function | Purpose |
|---|---|
| `getStore()` | Read `roamly_v2` from localStorage |
| `saveStore(d)` | Write `roamly_v2` to localStorage |
...
When a user shares an Instagram or TikTok post to Roamly, the app opens at `planner.html?title=...&text=...&url=...`. `handleIncomingShare()` reads these params, detects the source, extracts the place name, and opens the Quick Add modal pre-filled.
